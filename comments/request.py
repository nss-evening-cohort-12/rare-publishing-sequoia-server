    ...
    with sqlite3.connect("./rare.db") as conn: 
        ...
        UPDATE comments
            SET
                ...
                publication_date = ?
        WHERE id = ?
        """, (new_comment['subject'], new_comment['content'], new_comment['post_id'], new_comment['user_id'], new_comment['publication_date'], id, ))
...
def get_comment_by_id(id):
    with sqlite3.connect("./rare.db") as conn:
        ...
        comment = Comment(data['id'], data['subject'], data['content'], data['post_id'], data['user_id'], data['publication_date'])
